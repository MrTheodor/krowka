...
{   
    ...
    public class N2_file
    {
        ...
        public List<List<int> > Get_decode_description()
        {
            List<List<int> > ret = new List<List<int> >();
            Alphabet m_alp = new Alphabet();
            List<string> nasza = new List<string>();
            nasza = Get_description();
            foreach (string line in nasza)
            {
                List<int> ret_line = new List<int>();
                foreach (char znak in line)
                {
                    if (znak != '-')
                        ret_line.Add(m_alp.decode(znak.ToString()));
                    else
                        ret_line.Add(-1);
                }
                ret.Add(ret_line);
        ...
        public List<string> parse_file(string[] file) //parsing file
        {
            ...
            try
            {
                foreach (string line in file)
                {
                    if (counter == 0)
                        if (line != "FQT")
                            throw new System.ArgumentException("Invalid FQT function format, can't parse it");
                    if (counter == 1)
                    {
    ...
    public class automat
    {
        ...
        /// <summary>
        /// resizing world, if new size is greater than previous, the old world
        /// would by put on the center of new world. If is smaller, the new world 
        /// would consist center part of old world. All information on the edges would be lost
        /// </summary>
        ...
        public void resize_grid(int world_size)
        {
            ...
            if (world_size > X)
            {
                for (int y = 0; y < Y; y++)
                    for (int x = 0; x < X; x++)
                    {
                        tmp_siatka[x + (center / 2)-1, y + (center / 2)-1] = m_siatka[x, y];
                    }
            }
            else
            {
                int x = 0;
                int y = 0;
                for (int pY = (Y / 2) - center; pY < (Y / 2) + center; pY++)
                {
                    for (int pX = (X / 2) - center; pX < (X / 2) + center; pX++)
                    {
                        tmp_siatka[x, y] = m_siatka[pX, pY];
                    }
                }
            }
            m_siatka = tmp_siatka;
        ...
        public void randomize()
        {
            ...
            {
                for (Int32 y = 0; y < Y; y++)
                    SetCell(x, y, random.Next(alphabet.All().Count));
            }
        }
    ...
    public class LocalObservers
    {
        ...
        /// <summary>
        /// change the size of the world, it implicates also change all of the saved states
        /// </summary>
        /// <param name="world_size"></param>
        public void resize_world(int world_size)
        {
            for (int i = 0; i < myWorld_states.Count; i++)
            {
                int[,] tmp_siatka = new int[world_size, world_size];
                int center = world_size / 2;
                if (world_size > myWorld_size)
                {


                    for (int y = 0; y < myWorld_size; y++)
                        for (int x = 0; x < myWorld_size; x++)
                        {
                            tmp_siatka[x + (center / 2), y + (center / 2)] = myWorld_states[i][x, y];
                        }
                }
                else
                {
                    int x = 0;
                    int y = 0;
                    for (int pY = (myWorld_size / 2) - center; pY < (myWorld_size / 2) + center; pY++)
                    {
                        for (int pX = (myWorld_size / 2) - center; pX < (myWorld_size / 2) + center; pX++)
                        {
                            tmp_siatka[x, y] = myWorld_states[i][pX, pY];
                        }
                    }
                }
                myWorld_states[i] = tmp_siatka;
            }
            myWorld_size = world_size;
            myGridSize = world_size - 1;
            current_state();
        }
        ...
        private void current_state()
        {
            if (current_observer != 0)
            {
                ...
                for (int i = myStateCounter-1; i >= 0; i=i-light_speed) // loop for ligh speed
        ...
        public int[,] get_act_state()
        {
            return act_state;
        }

        public int GetPrevCell(int ix, int iy)
        {
            if (ix > myWorld_size || ix < 0 || iy >= myWorld_size || iy < 0)
                return 0;
            else
                return myWorld_states[0][ix, iy];
        }
